...
describe("Updates Page — Beta v1.0.2 & Product Release Archive", () => {
  it("renders Beta v1.0.2 as the latest release with three bullet points", () => {
    render(<UpdatesPage />);
    expect(screen.getByText("Beta v1.0.2")).toBeDefined();
    expect(
      screen.getByText("AWS Architecture Decoupling & Deployment Lifecycle")
    ).toBeDefined();
    expect(screen.getByText("Latest")).toBeDefined();

    const latestArticle = screen.getByText("Beta v1.0.2").closest("article");
    expect(latestArticle).not.toBeNull();
    const listItems = latestArticle!.querySelectorAll("li");
    expect(listItems).toHaveLength(3);
    expect(latestArticle?.textContent).toContain("Milestone & Architecture Decoupling");
    expect(latestArticle?.textContent).toContain("AWS Deployment Lifecycle");
    expect(latestArticle?.textContent).toContain("Architecture Flow Inspector");
  });

  it("renders Beta v1.0.1, Beta v1.0.0 and Alpha releases down to Alpha v1.0.0", () => {
    render(<UpdatesPage />);
