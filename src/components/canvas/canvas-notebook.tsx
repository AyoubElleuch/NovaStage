...
export default function CanvasNotebook({ projectId, isOpen, onToggle, onClose }: CanvasNotebookProps) {
  ...
  return (
    <>
      {isOpen &&
        createPortal(
          <aside aria-label="Project notebook" className="fixed top-0 right-0 z-30 flex h-dvh w-full max-w-105 flex-col border-l border-neutral-200 bg-white/95 shadow-2xl backdrop-blur-xl">
            <div className="flex items-center justify-between border-b border-neutral-100 px-5 py-4">
              <div className="flex items-center gap-2">
                <BookOpen className="h-4 w-4 text-neutral-500" aria-hidden="true" />
                <h2 className="text-sm font-semibold text-neutral-900">Project notebook</h2>
              </div>
              <div className="flex items-center gap-1.5">
                <button type="button" onClick={addEntry} className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-lg bg-neutral-900 px-2.5 text-xs font-semibold text-white hover:bg-neutral-800">
                  <Plus className="h-3.5 w-3.5" aria-hidden="true" /> New
                </button>
                <button type="button" onClick={onClose} title="Close notebook" aria-label="Close notebook" className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg text-neutral-400 hover:bg-neutral-100 hover:text-neutral-900">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
            ...
            <div className="border-b border-neutral-100 px-5 pt-3">
              <div className="flex gap-5" role="tablist" aria-label="Notebook sections">
                {(Object.keys(tabDetails) as NotebookTab[]).map((tab) => (
                  <button key={tab} type="button" role="tab" aria-selected={activeTab === tab} onClick={() => selectTab(tab)} className={`cursor-pointer border-b-2 pb-3 text-xs font-semibold transition-colors ${activeTab === tab ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-400 hover:text-neutral-700"}`}>
                    {tabDetails[tab].label}<span className="ml-1.5 text-[10px] text-neutral-400">{entries[tab].length}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {activeEntries.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                <BookOpen className="h-6 w-6 text-neutral-300" aria-hidden="true" />
                <h3 className="mt-3 text-sm font-semibold text-neutral-800">No {tabDetails[activeTab].label.toLowerCase()} yet</h3>
                <p className="mt-1 max-w-60 text-xs leading-5 text-neutral-400">Add one to preserve context alongside this project roadmap.</p>
                <button type="button" onClick={addEntry} className="mt-4 inline-flex h-9 cursor-pointer items-center gap-1.5 rounded-lg bg-neutral-900 px-3 text-xs font-semibold text-white hover:bg-neutral-800">
                  <Plus className="h-3.5 w-3.5" aria-hidden="true" /> New {tabDetails[activeTab].singular}
                </button>
              </div>
            ) : (
              <div className="flex min-h-0 flex-1 flex-col">
                <div aria-label={`${tabDetails[activeTab].label} history`} className="max-h-44 overflow-y-auto border-b border-neutral-100 p-3">
                  <div className="space-y-1">
                    {activeEntries.map((entry) => (
                      <div key={entry.id} className={`group flex items-center gap-2 rounded-lg px-2 py-1 ${selectedEntry?.id === entry.id ? "bg-neutral-100" : "hover:bg-neutral-50"}`}>
                ...
                {selectedEntry && (
                  <div className="flex min-h-0 flex-1 flex-col px-5 py-4">
                    ...
                    <input id="notebook-entry-title" value={selectedEntry.title} onChange={(event) => updateEntry({ title: event.target.value })} placeholder={`${tabDetails[activeTab].singular} title`} className="border-0 bg-transparent text-sm font-semibold text-neutral-900 outline-none placeholder:text-neutral-300" />
                    <label htmlFor="notebook-entry-body" className="sr-only">Entry content</label>
                    <textarea id="notebook-entry-body" value={selectedEntry.body} onChange={(event) => updateEntry({ body: event.target.value })} placeholder={tabDetails[activeTab].placeholder} className="mt-3 min-h-0 flex-1 resize-none rounded-lg border border-neutral-200 bg-neutral-50/60 p-3 text-sm leading-6 text-neutral-800 outline-none transition-colors placeholder:text-neutral-400 focus:border-neutral-400 focus:bg-white" />
                    <p className="mt-3 text-[11px] text-neutral-400">Private to this browser and saved automatically.</p>
                  </div>
                )}
              </div>
            )}
